...
@WebServlet("/downloadbytag")
public class DownloadByTag extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			String criteria = "Tag: " + request.getParameter("number");
			...
			sql += " WHERE sighted_butterflies.tag_id=" + request.getParameter("number") + " ORDER BY sight_date";
			resultSet = statement.executeQuery(sql);
			if (!resultSet.next()) {
				response.setContentType("text/html");
				PrintWriter out = response.getWriter();
				out.println("<font color=\"red\">No matching records found!</font><br><br>");
				request.getRequestDispatcher("downloadbytag.jsp").include(request, response);
				return;
			}
			...
			String fileName = "DownloadByTag " + dateFormat.format(new Date()) + ".txt";
