 ...
 */
@WebServlet("/deletetagging")
public class DeleteTagging extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (!user.isAdministrator()) {
			...
			try {
				...
				int tagID = Integer.parseInt(request.getParameter("number"));
				String sql = "DELETE FROM tagged_butterflies WHERE tag_id=?";
				preparedStatement = new ButterflyConnection().getConnection().prepareStatement(sql);
				preparedStatement.setInt(1, tagID);
				int rowsUpdated = preparedStatement.executeUpdate();
				if (rowsUpdated == 1)
					out.println("<font color=\"red\">Tag number " + tagID + " was removed successfully!</font><br>");
				else
					out.println("<font color=\"red\">The tag number " + tagID + " does not exist!</font><br>");
